...
template<class T>
void Keeper<T>:: search() {
	string sname;
	int cnt = 0;
	cout << "surname: ";
	cin >> sname;
	if (size == 0)
		cout << "Nothing search." << endl;
	else {

		for (int i = 0; i < size; i++) {

			if (ptr_mas[i].search(ptr_mas[i], sname))
			{
				cout << ptr_mas[i] << endl;
			}
			else {
...
void Keeper<T>::search1() {
	...
	if (size == 0)
		...
		for (int j = 0; j < size; j++) {
			...
			if (ptr_mas[j].search1(ptr_mas[j], sex))
			{
				cout << ptr_mas[j] << endl;
			}
			else {
